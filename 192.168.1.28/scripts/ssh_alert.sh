...
count=`cat $log_file | grep -v snoopy | grep -e "Failed" -e "Invalid" | grep -i "$fecha" | wc -l`
result=`cat $log_file | grep -v snoopy | grep -e "Failed" -e "Invalid" | grep -i "$fecha" > /tmp/ssh_mail.tmp`
