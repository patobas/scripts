...
;;
  ...
  2)  	clear
	...
	echo "* Descargamos repositorio.tar.gz de repo.renatea.gob.ar"
	...
	mkdir /root/scripts/
	cd /root/
	wget http://repo.renatea.gob.ar/repositorio.tar.gz
...
;;
  ...
  7)  	echo ""
	...
	postfix=`cat /etc/postfix/transport | grep smtp:mail.renatea.gob.ar | wc -l`
        if [ "$postfix" = "0" ] ; then
	echo ""
	echo ""
        read -p "Presione Enter para continuar..."
...
;;
  ...
  8)  	echo ""
	...
	cp $repo/check_mysql $repo/check_uptime $repo/check_memory /usr/lib/nagios/plugins/
